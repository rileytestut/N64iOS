...
#else
  sprintf(buffer, "%s", applicationDirectory);
  return true;
#endif
}
#endif
...
int         g_OsdEnabled = 0;           // On Screen Display enabled?
...
#if defined(__APPLE__) && defined(__arm__)
    sprintf(l_ConfigDir, "%s/config/", applicationDirectory);
  strncpy(l_ConfigDir, l_ConfigDir, PATH_MAX);
    sprintf(l_InstallDir, "%s/", applicationDirectory);
  strncpy(l_InstallDir, l_InstallDir, PATH_MAX);
  return;
