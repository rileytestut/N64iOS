...
EGLNativeWindowType     Handler; //Handle;
...
bool OGL_Start()
{
	...
	SDL_GetWMInfo(&info);
	Handler = (EGLNativeWindowType) info.window;
	Device = GetDC(Handler);
#else
	  //Handler = NULL;
    //Device = 0;
#endif
    ...
    Surface = eglCreateWindowSurface(Display, Config, Handler, NULL);
